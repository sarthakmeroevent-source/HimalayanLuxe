  ...
  const options = [
    {
      ...
      icon: (
        <svg viewBox="0 0 32 32" className="w-4 h-4 md:w-5 md:h-5 fill-white">
    ...
    },
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" className="w-4 h-4 md:w-5 md:h-5 fill-none stroke-white" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    ...
    >
      ...
      >
        <svg
          viewBox="0 0 24 24"
          className={`w-5 h-5 md:w-6 md:h-6 transition-all duration-300 ${open ? 'fill-white' : 'fill-black'}`}
        >
      ...
      {options.map((opt, i) => (
        <button
          key={opt.label}
          onClick={opt.onClick}
          className={`w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 ${opt.color} ${opt.hoverGlow} ${
            open
